...
from mangum import Mangum  # WSGI/ASGI handler for PythonAnywhere
...
# Change this to your PythonAnywhere URL if needed
...
# Paths (PythonAnywhere friendly)
PROJECT_ROOT = "/home/yourusername/astral_server"
...
TEMPERATURE = 0.7
TOP_P = 0.9
CPU_THREADS = 1  # reduce for PythonAnywhere free
...
def chat(msg: Message):
    ...
    mem_text = ''
    if relevant:
        mem_lines = [f"- ({m.get('role','mem')}) {m.get('text','')}" for m in relevant]
...
# -------------------------------
# WSGI HANDLER FOR PYTHONANYWHERE
# -------------------------------
handler = Mangum(app)